CREATE OR REPLACE FUNCTION public.__1_envurb_exclusion(
	schema_prod text,
	schema_data text,
	schema_bdtopo text,
	idcom text,
...
begin
	...
	--Ajout des zones à exclure (suppression dans la couche envurb)
	EXECUTE 'DROP TABLE IF EXISTS ' || schema_prod || '.exclusion_temp CASCADE';	
	EXECUTE 'CREATE TABLE ' || schema_prod || '.exclusion_temp AS
		SELECT ST_Makevalid(ST_Intersection(a.geom,ST_BUFFER(b.geom,10))) AS geom
		FROM ' || schema_data || '.zones_exclusion a,' || schema_bdtopo || '.' || bdt_com || ' b
		WHERE insee_com = ''' || idcom || ''' AND ST_Intersects(a.geom,b.geom)';
	EXECUTE 'CREATE INDEX ON ' || schema_prod || '.exclusion_temp USING gist(geom)';
	
	EXECUTE 'DROP TABLE IF EXISTS ' || schema_prod || '.exclusion_temp2 CASCADE';	
	EXECUTE 'CREATE TABLE ' || schema_prod || '.exclusion_temp2 AS
		SELECT st_multi(st_collectionextract(st_forcecollection(st_makevalid(st_difference((st_multi(st_collectionextract(st_forcecollection(st_makevalid(st_union(a.geom))),3))),(st_multi(st_collectionextract(st_forcecollection(st_makevalid(st_union(b.geom))),3)))))),3))::geometry (MultiPolygon,2154) as geom
		FROM ' || schema_prod || '.gf_envurb_' || idcom || ' a
		JOIN ' || schema_prod || '.exclusion_temp b
		ON ST_Intersects(a.geom,b.geom)';
	
	--	remplacement de la table envurb
	EXECUTE 'DROP TABLE IF EXISTS ' || schema_prod || '.gf_envurb_' || idcom || '';
	EXECUTE 'CREATE TABLE ' || schema_prod || '.gf_envurb_' || idcom || ' AS
		SELECT 
			row_number() over() as gid,
			geom from
		' || schema_prod || '.exclusion_temp2'	;
...
--SELECT public.__1_envurb_exclusion('public','r_ign_bdtopo','33001','commune_d33','envurb_33','plu_33',500,50);